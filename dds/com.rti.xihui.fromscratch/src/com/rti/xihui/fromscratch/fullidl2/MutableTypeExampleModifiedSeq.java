...
/**
 * A sequence of MutableTypeExampleModified instances.
 */
public final class MutableTypeExampleModifiedSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public MutableTypeExampleModifiedSeq() {
        super(MutableTypeExampleModified.class);
    }


    public MutableTypeExampleModifiedSeq(int initialMaximum) {
        super(MutableTypeExampleModified.class, initialMaximum);
    }


    public MutableTypeExampleModifiedSeq(Collection elements) {
        super(MutableTypeExampleModified.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, MutableTypeExampleModified.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' MutableTypeExampleModified objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MutableTypeExampleModified.create());
