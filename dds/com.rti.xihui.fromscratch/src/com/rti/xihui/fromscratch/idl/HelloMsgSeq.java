...
/**
 * A sequence of HelloMsg instances.
 */
public final class HelloMsgSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public HelloMsgSeq() {
        super(HelloMsg.class);
    }


    public HelloMsgSeq(int initialMaximum) {
        super(HelloMsg.class, initialMaximum);
    }


    public HelloMsgSeq(Collection elements) {
        super(HelloMsg.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, HelloMsg.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' HelloMsg objects (beyond the original size of this object)
