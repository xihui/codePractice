...
/**
 * A sequence of ServiceType instances.
 */
public final class ServiceTypeSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public ServiceTypeSeq() {
        super(ServiceType.class);
    }


    public ServiceTypeSeq(int initialMaximum) {
        super(ServiceType.class, initialMaximum);
    }


    public ServiceTypeSeq(Collection elements) {
        super(ServiceType.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, ServiceType.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' ServiceType objects (beyond the original size of this object)
