...
/**
 * A sequence of MutableTypeExample instances.
 */
public final class MutableTypeExampleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public MutableTypeExampleSeq() {
        super(MutableTypeExample.class);
    }


    public MutableTypeExampleSeq(int initialMaximum) {
        super(MutableTypeExample.class, initialMaximum);
    }


    public MutableTypeExampleSeq(Collection elements) {
        super(MutableTypeExample.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, MutableTypeExample.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' MutableTypeExample objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MutableTypeExample.create());
