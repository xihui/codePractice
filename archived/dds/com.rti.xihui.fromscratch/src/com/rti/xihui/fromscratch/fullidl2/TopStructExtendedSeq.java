...
/**
 * A sequence of TopStructExtended instances.
 */
public final class TopStructExtendedSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public TopStructExtendedSeq() {
        super(TopStructExtended.class);
    }


    public TopStructExtendedSeq(int initialMaximum) {
        super(TopStructExtended.class, initialMaximum);
    }


    public TopStructExtendedSeq(Collection elements) {
        super(TopStructExtended.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, TopStructExtended.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' TopStructExtended objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TopStructExtended.create());
