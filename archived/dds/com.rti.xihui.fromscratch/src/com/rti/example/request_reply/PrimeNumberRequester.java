...
import com.rti.xihui.fromscratch.idl.HelloMsg;
import com.rti.xihui.fromscratch.idl.HelloMsgDataReader;
import com.rti.xihui.fromscratch.idl.HelloMsgSeq;
...
public class PrimeNumberRequester {
    ...
    public void printPrimeNumbers(int n, int primesPerReply) 
        ...
        System.out.println("request:"+ request.getIdentity());

        // Create a request sample
        final WriteSample<PrimeNumberRequest> request2 = 
                requester.createRequestSample();
        
        request2.getData().n = 20;
        request2.getData().primes_per_reply = primesPerReply;
        requester.sendRequest(request2);
        System.out.println("request2:"+ request2.getIdentity());
        
        requester.getReplyDataReader().set_listener(new DataReaderAdapter() {
    		private PrimeNumberReplySeq msgSeq = new PrimeNumberReplySeq();
    		private SampleInfoSeq infoSeq = new SampleInfoSeq();

    		@Override
    		public void on_data_available(DataReader reader) {
    			try {
    				((PrimeNumberReplyDataReader) reader).read(msgSeq, infoSeq,
    				...
    				for (int i = 0; i < msgSeq.size(); i++) {					
    					...
    					System.out.println(
    							"writer_guid" + info.related_original_publication_virtual_guid + " "
    									+ "sequence number: " + info.related_original_publication_virtual_sequence_number);
    					
    					if (info.valid_data)
    						System.out.println((PrimeNumberReply) msgSeq.get(i));
    					else{
...
//    				Thread.sleep(1000);
    				...
    				((PrimeNumberReplyDataReader) reader).return_loan(msgSeq, infoSeq);
    			}
    		}
        ...
        while(inProgress) {
            ...
            try {
                do { // Iterate through the sample list
                    ...
                    System.out.println("reply from " + reply.getIdentity());
